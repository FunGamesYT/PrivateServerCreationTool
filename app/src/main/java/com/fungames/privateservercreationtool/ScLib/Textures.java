...
public class Textures
{
    ...
    /// <returns></returns>
    public static Bitmap GetBitmapBySc(InputStream inStream, String fileName) throws IOException {
        byte id = (byte) inStream.read();
        inStream.skip(4);
        int pxFomat = inStream.read();
        byte[] byteArr = new byte[2];
        inStream.read(byteArr);
        int width = ByteBuffer.wrap(byteArr).order(ByteOrder.BIG_ENDIAN).getInt();
        inStream.read(byteArr);
        int heigth = ByteBuffer.wrap(byteArr).order(ByteOrder.BIG_ENDIAN).getInt();

        int mtWidth = id == 1 ? width : width % 32;
        int ttWidth = id == 1 ? width - mtWidth : (width - mtWidth) / 32;
        int mtHeigth = id == 1 ? heigth : heigth % 32;
        int ttHeigth = id == 1 ? heigth - mtHeigth : (heigth - mtHeigth) / 32;

        int[][] pixelArray = new int[heigth][width];
        ...
        for (int index = 0; index < ttHeigth + 1; index++) {
            ...
            for (int row = 0; row < pixelArray[0].length; row++) {
                for (int column = 0; column < pixelArray[1].length; column++) {
    ...
    /// <returns></returns>
    private static int GetColorByPxFormat(InputStream reader, int pxFormat) throws IOException {
        int color;
